...
buf4[].  Below is a high-level example of how read4 works:
...
characters read4(buf4); // read4 returns 4. Now buf4 = "abcd", fp points to 'e'
...
method, buf should contain "abc". We read a total of 3 characters from the file,
so return 3.  Note that "abc" is the file's content, not buf. buf is the
...
Input: file = "abcdABCD1234", n = 12 Output: 12 Explanation: After calling your
read method, buf should contain "abcdABCD1234". We read a total of 12 characters
from the file, so return 12.  Example 4:

Input: file = "leetcode", n = 5 Output: 5 Explanation: After calling your read
method, buf should contain "leetc". We read a total of 5 characters from the
file, so return 5.
...
// Using internal buffer of 4 characters
const solution = (read4) => {
  const read = (buf, n) => {
    let charactersRead = 4;
    let numberOfCopiedCharacters = 0;
    const buf4 = [];

    while (numberOfCopiedCharacters < n && charactersRead === 4) {
      charactersRead = read4(buf4);

      for (let i = 0; i < charactersRead; i += 1) {
        if (numberOfCopiedCharacters === n) return numberOfCopiedCharacters;

        buf[numberOfCopiedCharacters] = buf4[i];
        numberOfCopiedCharacters += 1;
      }
    }

    return numberOfCopiedCharacters;
  };
