...
The number of nodes in the tree is in the range [1, 5000].  1 <= Node.val <= 107
root is a binary search tree.  1 <= val <= 107 */

const searchBST = (root, val) => {
  if (!root || root.val === val) return root;

  // Will not enter the second return if it goes in the first one
  // Easy to also write as a ternary
