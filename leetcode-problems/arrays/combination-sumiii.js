...
Input: k = 3, n = 9 Output: [[1,2,6],[1,3,5],[2,3,4]] Explanation: 1 + 2 + 6 = 9
1 + 3 + 5 = 9 2 + 3 + 4 = 9 There are no other valid combinations.  Example 3:

Input: k = 4, n = 1 Output: [] Explanation: There are no valid combinations.
Using 4 different numbers in the range [1,9], the smallest sum we can get is
1+2+3+4 = 10 and since 10 > 1, there are no valid combination.  Example 4:

Input: k = 3, n = 2 Output: [] Explanation: There are no valid combinations.
Example 5:

Input: k = 9, n = 45 Output: [[1,2,3,4,5,6,7,8,9]] Explanation: 1 + 2 + 3 + 4 +
5 + 6 + 7 + 8 + 9 = 45 There are no other valid combinations.
...
const combinationSum3 = (k, n) => {
  ...
  const backtrack = (remain, i) => {
    if (temp.length <= k) {
      if (remain === 0 && temp.length === k) {
        result.push([...temp]);
      } else if (remain > 0) {
        for (let j = i; j < options.length; j += 1) {
          const curr = options[j];
          temp.push(curr);
