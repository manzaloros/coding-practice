...
// Time: O (log n), but worst case is all same number, then it's O(n)
// Space: O(1)
const findMin = (nums) => {
  let [left, right] = [0, nums.length - 1];

  while (left < right) {
    // Need these inner parentheses in the floor, otherwise division will
    // happen before subtraction
