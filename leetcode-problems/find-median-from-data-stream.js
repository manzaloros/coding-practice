...
medianFinder.addNum(1);    // arr = [1] medianFinder.addNum(2);    // arr = [1,
2] medianFinder.findMedian(); // return 1.5 (i.e., (1 + 2) / 2)
...
optimize your solution?  If 99% of all integer numbers from the stream are in
the range [0, 100], how would you optimize your solution?
 */
// Input isn't sorted... So you'd have to sort EVERY time you get the median.
...
class MedianFinder {
  ...
  addNum(num) {
    const { lo, hi } = this;
    lo.enqueue(num);
    const { element } = lo.dequeue();

    hi.enqueue(element);

    if (lo.size() < hi.size()) {
      const dequeued = hi.dequeue();
      lo.enqueue(dequeued.element);
    }
  }
