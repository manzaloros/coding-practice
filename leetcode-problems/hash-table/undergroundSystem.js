...
class UndergroundSystem {
  ...
  checkIn(id, stationName, time) {
    this.customers.set(id, [stationName, time]);
  }

  checkOut(id, stationName, time) {
    const totalTime = time - this.customers.get(id)[1];
    const startStation = this.customers.get(id)[0];

    if (!this.stations.has(startStation)) {
      this.stations.set(startStation, [[stationName, totalTime, totalTime, 1]]);
    } else {
      ...
      this.stations.get(startStation).push([stationName, totalTime, totalTime, 1]);
    }
