...
MinStack() initializes the stack object.  void push(val) pushes the element val
onto the stack.  void pop() removes the element on the top of the stack.  int
top() gets the top element of the stack.  int getMin() retrieves the minimum
...
class MinStack {
  ...
  push(val) {
    const min = this.storage.length === 0 ? val : Math.min(this.getMin(), val);
    const node = [val, min];
