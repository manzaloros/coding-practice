...
const explode = (node) => {
  const _explode = (node, prevNode, addVal, dir, traversingUp) => {
    ...
    if (node[dir] !== prevNode) {
      if (traversingUp && node[dir].val === -1) { _explode(node[dir], null, addVal, dir === 'left' ? 'right' : 'left', false); } else if (node[dir].val === -1) _explode(node[dir], null, addVal, dir, traversingUp);
