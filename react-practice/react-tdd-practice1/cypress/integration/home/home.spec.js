...
/* Describe() and it() come from Mocha, expect() comes from chai
  cy.get() returns the DOM element(s) it finds matching selector

  Cypress chains commands together by managing a promise chain, with each command yielding a 'subject' to the next command

  Selectors are based off of jQuery selectors
*/
describe("Home page", () => {
  beforeEach(() => {
    cy.visit('/');
